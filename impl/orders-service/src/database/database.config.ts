...
export class DatabaseConfigProvider {
    ...
    public static get = (): DatabaseConfig => {
        ...
        if (dialect === 'sqlite') {
            return {
                Dialect    : 'sqlite',
                SqliteFile : process.env.ORDERS_DB_FILE ?? './data/orders.sqlite',
        ...
        const url = process.env.ORDERS_DB_URL ?? process.env.DATABASE_URL;
        ...
        return {
            ...
            Database   : process.env.ORDERS_DB_NAME ?? process.env.DB_NAME ?? 'orders_db',
