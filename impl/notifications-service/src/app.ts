...
export default class Application {
    ...
    public start = async (): Promise<void> => {
        try {
            ...
            logger.error('An error occurred while starting notifications-service. ' + error.message);
        }
    };
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const server = this._app.listen(port, () => {
                    const serviceName = `notifications-service-[${process.env.NODE_ENV ?? 'development'}]`;
                    logger.info('-----------------------------------------------------------');
