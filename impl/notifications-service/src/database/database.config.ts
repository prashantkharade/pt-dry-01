...
export class DatabaseConfigProvider {
    ...
    public static get = (): DatabaseConfig => {
        ...
        if (dialect === 'sqlite') {
            return {
                Dialect    : 'sqlite',
                SqliteFile : process.env.NOTIFICATIONS_DB_FILE ?? './data/notifications.sqlite',
        ...
        const url = process.env.NOTIFICATIONS_DB_URL ?? process.env.DATABASE_URL;
        ...
        return {
            ...
            Database   : process.env.NOTIFICATIONS_DB_NAME ?? process.env.DB_NAME ?? 'notifications_db',
