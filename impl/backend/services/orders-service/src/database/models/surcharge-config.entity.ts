...
export class SurchargeConfig {
  ...
  @Column({ name: 'Key', type: 'varchar', length: 64 }) Key!: string;
  @Column({ name: 'Value', type: 'numeric', precision: 15, scale: 4 }) Value!: string;
  @Column({ name: 'Unit', type: 'varchar', length: 16 }) Unit!: string; // INR | PCT
