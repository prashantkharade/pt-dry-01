...
export class DatabaseConfigProvider {
    ...
    public static get = (): DatabaseConfig => {
        ...
        if (dialect === 'sqlite') {
            return {
                Dialect    : 'sqlite',
                SqliteFile : process.env.PAYMENTS_DB_FILE ?? './data/payments.sqlite',
        ...
        const url = process.env.PAYMENTS_DB_URL ?? process.env.DATABASE_URL;
        ...
        return {
            ...
            Database   : process.env.PAYMENTS_DB_NAME ?? process.env.DB_NAME ?? 'payments_db',
